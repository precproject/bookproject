    ...
    >
      ...
      <motion.div style={{ opacity }} className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-16 xl:gap-20">
          ...
          <motion.div 
            style={{ y: textY }} 
            className="w-full lg:w-1/2 space-y-6 md:space-y-8 text-center lg:text-left flex flex-col items-center lg:items-start z-20 order-1 px-2 sm:px-0"
          >
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-wrap items-center justify-center lg:justify-start gap-2">
              <span className="px-4 py-1.5 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 rounded-full text-[10px] md:text-xs font-mukta font-black uppercase tracking-widest border border-orange-200 dark:border-orange-800 shadow-sm">
            ...
            {/* Title (FIXED: Removed lg:hidden bug) */}
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-rozha font-bold leading-[1.15] w-full">
              <span className="text-4xl sm:text-5xl lg:text-[3.5rem] xl:text-6xl text-slate-900 dark:text-white block mb-2 lg:mb-3">
                {t('mainHero.titlePart1', 'वारसा पराक्रमाचा,')}
              </span>
              <span className="gold-gradient-text drop-shadow-sm dark:drop-shadow-none text-[2.75rem] leading-[1.1] sm:text-6xl md:text-7xl lg:text-[4.25rem] xl:text-[5.5rem] block">
               {t('mainHero.titlePart2', 'चिंतनातून चिंतामुक्तीकडे')}
              </span>
            </motion.h1>

            {/* Inspirational Quote Box (FIXED: Full width & scaling on Desktop) */}
            <motion.div 
              initial={{ opacity: 0, y: 20 }} 
              animate={{ opacity: 1, y: 0 }} 
              transition={{ delay: 0.2 }}
              className="relative w-full max-w-[95%] sm:max-w-[90%] lg:max-w-xl mx-auto lg:mx-0 px-5 md:px-6 py-3 md:py-4 border-l-4 border-orange-500 bg-slate-100 dark:bg-slate-800/60 rounded-r-xl shadow-sm"
            >
              <p className="text-[14px] sm:text-lg lg:text-xl text-slate-700 dark:text-slate-300 leading-relaxed md:leading-relaxed font-mukta font-medium italic text-left">
            ...
            >
              ...
              {isPrebookActive ? (
                  ...
                  className="relative overflow-hidden group flex-1 sm:flex-none sm:w-auto px-4 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-orange-500 to-[#FF5A36] text-white rounded-full font-bold text-sm sm:text-lg shadow-[0_10px_30px_rgba(255,90,54,0.3)] hover:shadow-[0_15px_40px_rgba(255,90,54,0.5)] transition-all duration-300 transform hover:-translate-y-1 flex items-center justify-center gap-2 border border-orange-400/50 active:scale-95"
                >
                  <span className="relative z-10 flex items-center gap-2 whitespace-nowrap">
                    <BellRing size={18} className="sm:w-5 sm:h-5 group-hover:rotate-12 transition-transform" /> 
