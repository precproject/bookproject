...
export const Author = () => {
  ...
  // A clean, reliable animation variant for text blocks
  const fadeUpVariant = {
    ...
    <section id="author" className="py-16 md:py-24 lg:py-32 relative overflow-hidden z-0 transition-colors duration-300">
      ...
      <div className="container mx-auto px-4 md:px-6 flex flex-col md:flex-row items-center justify-between gap-12 md:gap-24 lg:gap-32 max-w-7xl relative z-10">
        ...
        <motion.div 
          ...
          className="md:hidden text-center w-full flex flex-col items-center mb-2 px-2"
        >
          <span className="inline-block mb-3 px-4 py-1.5 rounded-full border border-orange-200 dark:border-orange-800 bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 text-xs font-mukta font-bold uppercase tracking-widest shadow-sm">
            {t('author.sectionLabel', 'लेखक')}
          </span>

          {/* Removed whitespace-nowrap, adjusted font size to fit neatly or wrap by word */}
          <h3 className="text-2xl sm:text-3xl gold-gradient-text drop-shadow-sm dark:drop-shadow-none font-rozha font-bold py-1 w-full leading-tight">
        ...
        <div className="w-full md:w-3/5 lg:w-3/5 text-center md:text-left mt-8 md:mt-0">
          ...
          >
            ...
            {/* Removed whitespace-nowrap to prevent overflow. Font scales cleanly. */}
            <h3 className="gold-gradient-text drop-shadow-sm dark:drop-shadow-none py-2 text-3xl lg:text-4xl xl:text-5xl font-rozha font-bold leading-tight tracking-tight">
              {t('author.name', 'कैलासराव तुकाराम तुरकणे पाटील')}
            </h3>
          </motion.div>
