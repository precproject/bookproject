...
export const DashboardLayout = ({ children, activePath = '/admin' }) => {
  ...
  const handleLogout = () => {
    logout(); 
    navigate('/admin/login'); // Fixed redirect to point to the actual admin login route
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex font-sans text-slate-800">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-slate-100 px-4 py-6 h-screen sticky top-0 shrink-0 z-40">
        ...
        <button type="button" className="flex items-center gap-2 px-2 mb-10 text-emerald-800 hover:opacity-80 transition-opacity" onClick={() => navigate('/admin')}>
          <div className="w-8 h-8 rounded-xl bg-emerald-800 flex items-center justify-center text-white font-bold">S</div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
          ...
          <button type="button" onClick={() => navigate('/admin/settings')} className={`w-full flex items-center gap-3 px-3 py-3 rounded-2xl transition-all mb-2 ${activePath === '/admin/settings' ? 'bg-emerald-800 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        {/* Topbar */}
        <header className="h-20 bg-[#F8F9FA]/80 backdrop-blur-md sticky top-0 z-30 px-4 md:px-6 flex items-center justify-between shrink-0 border-b border-slate-100 md:border-none">
          
          <div className="flex items-center flex-1">
            <button type="button" onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 bg-white border border-slate-200 rounded-full shadow-sm text-slate-600 hover:text-emerald-700 transition-colors mr-4">
              <Menu size={20} />
            </button>
            ...
            <div className="hidden sm:flex items-center bg-white border border-slate-200 rounded-full px-4 py-2.5 w-full max-w-md focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all shadow-sm">
              ...
              <div className="hidden lg:flex items-center justify-center bg-slate-100 rounded text-[10px] font-bold text-slate-500 px-1.5 py-0.5 ml-2 shrink-0">⌘K</div>
            </div>
          </div>

          {/* CRITICAL FIX: Hide profile dropdown on mobile, force them to use sidebar */}
