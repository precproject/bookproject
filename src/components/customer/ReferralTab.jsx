...
const ReferralTab = () => {
  ...
  if (loading) return <div className="text-center py-20 text-slate-500">Loading referral stats...</div>;
  
  if (!referralData) {
    return (
      <div className="text-center py-16">
        <Share2 size={60} className="text-slate-300 mx-auto mb-4" />
        <h3 className="text-xl font-bold text-slate-700 mb-2">No Active Referrals</h3>
        <p className="text-slate-500">You are not currently enrolled in our referral program.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-3xl p-8 mb-8 text-center relative overflow-hidden">
        ...
        <h2 className="text-green-800 font-bold mb-2">Your Unique Referral Link</h2>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-4">
          ...
          <button onClick={copyToClipboard} className="bg-green-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-green-700 transition shadow-sm">
            Copy Link
          </button>
        </div>
        <p className="text-sm font-medium text-green-600 mt-4">Share this link to earn ₹{referralData.rewardRate} per successful book sale!</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        ...
        <div className="bg-orange-50 border border-orange-100 p-6 rounded-2xl shadow-sm text-center">
          ...
          <p className="text-orange-700 font-medium mb-1">Pending Payout</p>
        ...
        <div className="bg-green-50 border border-green-100 p-6 rounded-2xl shadow-sm text-center">
          ...
          <p className="text-green-700 font-medium mb-1">Successfully Paid</p>
      ...
      <h3 className="font-bold text-xl mb-6 text-slate-800 flex items-center gap-2">
        Successful Referrals <span className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-sm">{referralData.transactions?.length || 0}</span>
      </h3>
          ...
          <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Order Total</th>
                <th className="px-6 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Your Commission</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {referralData.transactions?.map((txn) => (
                <tr key={txn._id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">{txn.orderId}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">₹{txn.priceBreakup?.total || 0}</td>
              ...
              {(!referralData.transactions || referralData.transactions.length === 0) && (
                <tr>
                  <td colSpan="4" className="px-6 py-12 text-center text-slate-500">No successful referrals yet. Share your link to get started!</td>
                </tr>
              )}
            </tbody>
          </table>
