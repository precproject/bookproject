...
export const ProductPage = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center transition-colors duration-300">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium">{t('product.loading', 'Loading details...')}</p>
      </div>
    );
  }
  ...
  if (!book) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center transition-colors duration-300">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-4">{t('product.notFound', 'Book not found')}</h2>
        <Button onClick={() => navigate('/store')}>{t('product.returnStore', 'Return to Store')}</Button>
      </div>
    );
  }
      ...
      <main className="pt-24 pb-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <button onClick={() => navigate('/store')} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-orange-600 dark:text-slate-400 dark:hover:text-orange-500 transition-colors">
            <ArrowLeft size={16} /> {t('product.back', 'Back to Collection')}
          </button>
        </div>
              ...
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                <div className="mb-4 flex flex-wrap items-center gap-3">
                  <span className="px-3 py-1 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 rounded-full text-xs font-bold uppercase tracking-wider border border-orange-200 dark:border-orange-800">
                    {book.type} {t('product.edition', 'Edition')}
                  </span>
                  {book.type === 'Physical' && book.stock > 0 ? (
                    <span className="text-xs font-bold text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800/50 px-3 py-1 rounded-full flex items-center gap-1">
                      <CheckCircle size={14}/> {t('product.inStock', 'In Stock')}
                    </span>
                  ) : book.type === 'Physical' ? (
                ...
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <span className="text-sm font-bold text-slate-600 dark:text-slate-300 underline cursor-pointer hover:text-orange-600" onClick={() => document.getElementById('reviews').scrollIntoView({ behavior: 'smooth' })}>
                    {reviews.length} {t('product.verifiedReviews', 'Verified Reviews')}
                  </span>
                </div>
                ...
                <div className="mb-10">
                  {!hasReferral ? (
                    ...
                    <div className="p-6 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 text-center shadow-inner">
                      ...
                      <h3 className="text-lg font-bold text-slate-800 dark:text-white mb-2">{t('product.inviteOnly', 'Invite-Only Access')}</h3>
                      <p className="text-sm text-slate-500 dark:text-slate-400 max-w-md mx-auto">
                        {t('product.inviteOnlyDesc', 'This book is currently available exclusively through member referrals. Please click a valid referral link shared by a friend to unlock purchasing.')}
                      </p>
                    </div>
                  ) : cartItem ? (
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800">
                      ...
                      >
                        {t('product.proceedCheckout', 'Proceed to Checkout')} <ChevronRight size={18} />
                      </Button>
                    </div>
                  ) : (
                    ...
                    >
                      {isAdding ? <><Loader2 size={20} className="animate-spin" /> {t('product.addingToCart', 'Adding to Cart...')}</> : <><ShoppingBag size={20} /> {t('product.addToCart', 'Add to Cart')}</>}
                    </Button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 py-6 border-t border-slate-200 dark:border-slate-800">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">{t('product.language', 'Language')}</span>
                    <span className="text-sm font-bold text-slate-900 dark:text-white">{t('product.marathi', 'Marathi')}</span>
                  </div>
                  <div className="flex flex-col gap-1">
                    <Book className="text-slate-400 dark:text-slate-500 mb-1" size={20} />
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">{t('product.format', 'Format')}</span>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wider">{t('product.pages', 'Pages')}</span>
                ...
                <div className="grid grid-cols-2 gap-4 py-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 mt-4 rounded-2xl px-4">
                  <div className="flex items-center gap-3 text-slate-700 dark:text-slate-300">
                    ...
                    <span className="text-sm font-bold leading-tight">{t('product.fastDelivery', 'Fast Delivery')}<br/><span className="text-xs font-normal text-slate-500">{t('product.acrossMh', 'Across Maharashtra')}</span></span>
                  </div>
                  <div className="flex items-center gap-3 text-slate-700 dark:text-slate-300">
                    ...
                    <span className="text-sm font-bold leading-tight">{t('product.securePayment', 'Secure Payment')}<br/><span className="text-xs font-normal text-slate-500">{t('product.encrypted', '100% Encrypted')}</span></span>
                  </div>
                </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-900 border-y border-slate-200 dark:border-slate-800 mt-20 py-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
            <h2 className="text-2xl md:text-3xl font-serif font-bold text-slate-900 dark:text-white mb-8">{t('product.discover', "What you'll discover inside")}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white mb-2">{t('product.benefit1Title', 'Build Better Habits')}</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">{t('product.benefit1Desc', 'Learn actionable techniques to break bad habits and consistently stick to good ones.')}</p>
              </div>
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white mb-2">{t('product.benefit2Title', 'Financial Freedom')}</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">{t('product.benefit2Desc', 'Understand wealth-building strategies specifically tailored for modern thinkers.')}</p>
              </div>
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
                ...
                <h3 className="font-bold text-slate-900 dark:text-white mb-2">{t('product.benefit3Title', 'Clarity of Mind')}</h3>
                <p className="text-sm text-slate-600 dark:text-slate-400">{t('product.benefit3Desc', "Achieve 'Chintamukti' (Freedom from worry) through practical daily exercises.")}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div id="reviews" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20">
          <h2 className="text-3xl font-serif font-bold text-slate-900 dark:text-white mb-10">{t('product.customerReviews', 'Customer Reviews')}</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1 bg-slate-50 dark:bg-slate-900 p-8 rounded-3xl border border-slate-200 dark:border-slate-800 h-fit sticky top-32">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">{t('product.writeReview', 'Write a Review')}</h3>
              {!user ? (
                <div className="text-center py-6">
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{t('product.loginToShare', 'You must be logged in to share your thoughts on this book.')}</p>
                  <Button variant="secondary" onClick={() => navigate('/dashboard')} className="w-full">{t('product.loginToReview', 'Login to Review')}</Button>
                </div>
              ) : (
                <form onSubmit={handleSubmitReview} className="space-y-5">
                  <div>
                    <label className="text-sm font-bold text-slate-700 dark:text-slate-300 block mb-2">{t('product.rating', 'Rating')}</label>
                  ...
                  <div>
                    <label className="text-sm font-bold text-slate-700 dark:text-slate-300 block mb-2">{t('product.yourFeedback', 'Your Feedback')}</label>
                    <textarea 
                      required rows="4" 
                      value={newReview.comment} onChange={(e) => setNewReview({ ...newReview, comment: e.target.value })}
                      placeholder={t('product.reviewPlaceholder', 'What did you think about the book?')} 
                  ...
                  <Button variant="primary" type="submit" disabled={submittingReview} className="w-full py-3.5">
                    {submittingReview ? <><Loader2 size={18} className="animate-spin mr-2 inline"/> {t('product.submitting', 'Submitting...')}</> : t('product.submitReview', 'Submit Review')}
                  </Button>
                </form>
              )}
            </div>
